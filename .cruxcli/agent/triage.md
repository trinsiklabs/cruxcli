...
**Only** add if it's specifically about the Desktop application or `cruxcli web` view. **Do not** add for terminal, TUI, or general cruxcli issues.
...
**Only** add if the issue mentions "zen" or "cruxcli zen" or "cruxcli black".

If the issue doesn't have "zen" or "cruxcli black" in it then don't add zen label

#### core

Use for core server issues in `packages/cruxcli/`, excluding `packages/cruxcli/src/cli/cmd/tui/`.
...
TUI (`packages/cruxcli/src/cli/cmd/tui/...`):
...
Core (`packages/cruxcli/...`, excluding TUI subtree):

- thdxr for sqlite/snapshot/memory bugs and larger architectural core features
- jlongster for cruxcli server + API feature work (tool currently remaps jlongster -> thdxr until assignable)
