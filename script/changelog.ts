...
import { $ } from "bun"
import { createCruxcli } from "@cruxcli/sdk/v2"
...
export async function getLatestRelease(skip?: string) {
  const data = await fetch("https://api.github.com/repos/trinsiklabs/cruxcli/releases?per_page=100").then((res) => {
...
export async function getCommits(from: string, to: string): Promise<Commit[]> {
  ...
  const compare =
    await $`gh api "/repos/trinsiklabs/cruxcli/compare/${fromRef}...${toRef}" --jq '.commits[] | {sha: .sha, login: .author.login, message: .commit.message}'`.text()
  ...
  const log =
    await $`git log ${fromRef}..${toRef} --oneline --format="%H" -- packages/cruxcli packages/sdk packages/plugin packages/desktop packages/app sdks/vscode packages/extensions github`.text()
  ...
  for (const hash of hashes) {
    ...
    for (const file of files.split("\n").filter(Boolean)) {
      if (file.startsWith("packages/cruxcli/src/cli/cmd/")) areas.add("tui")
      else if (file.startsWith("packages/cruxcli/")) areas.add("core")
...
async function summarizeCommit(cruxcli: Awaited<ReturnType<typeof createCruxcli>>, message: string): Promise<string> {
  console.log("summarizing commit:", message)
  const session = await cruxcli.client.session.create()
  const result = await cruxcli.client.session
    .prompt(
      {
        sessionID: session.data!.id,
        model: { providerID: "cruxcli", modelID: "claude-sonnet-4-5" },
        tools: {
          "*": false,
        },
        parts: [
...
export async function generateChangelog(commits: Commit[], cruxcli: Awaited<ReturnType<typeof createCruxcli>>) {
  // Summarize commits in parallel with max 10 concurrent requests
  const BATCH_SIZE = 10
  const summaries: string[] = []
  for (let i = 0; i < commits.length; i += BATCH_SIZE) {
    const batch = commits.slice(i, i + BATCH_SIZE)
    const results = await Promise.all(batch.map((c) => summarizeCommit(cruxcli, c.message)))
...
export async function getContributors(from: string, to: string) {
  ...
  const toRef = to === "HEAD" ? to : to.startsWith("v") ? to : `v${to}`
  const compare =
    await $`gh api "/repos/trinsiklabs/cruxcli/compare/${fromRef}...${toRef}" --jq '.commits[] | {login: .author.login, message: .commit.message}'`.text()
...
export async function buildNotes(from: string, to: string) {
  ...
  const cruxcli = await createCruxcli({ port: 0 })
  const notes: string[] = []

  try {
    const lines = await generateChangelog(commits, cruxcli)
    notes.push(...lines)
