...
async function fix(pr: PR, files: string[]) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with cruxcli...`)
  const prompt = [
  ...
  try {
    await $`cruxcli run -m cruxcli/gpt-5.3-codex ${prompt}`
  } catch (err) {
    console.log(`  cruxcli failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with cruxcli")
  return true
}
