...
import { Splash } from "@cruxcli/ui/logo"
...
const deepLinkEvent = "cruxcli:deep-link"

const emitDeepLinks = (urls: string[]) => {
  if (urls.length === 0) return
  window.__CRUXCLI__ ??= {}
  const pending = window.__CRUXCLI__.deepLinks ?? []
  window.__CRUXCLI__.deepLinks = [...pending, ...urls]
  window.dispatchEvent(new CustomEvent(deepLinkEvent, { detail: { urls } }))
}

const listenForDeepLinks = () => {
  const startUrls = window.__CRUXCLI__?.deepLinks ?? []
...
const createPlatform = (): Platform => {
  ...
  const wslHome = async () => {
    if (os !== "windows" || !window.__CRUXCLI__?.wsl) return undefined
    return window.api.wslPath("~", "windows").catch(() => undefined)
  }

  const handleWslPicker = async <T extends string | string[]>(result: T | null): Promise<T | null> => {
    if (!result || !window.__CRUXCLI__?.wsl) return result
  ...
  return {
    ...
    notify: async (title, description, href) => {
      ...
      const notification = new Notification(title, {
        body: description ?? "",
        icon: "https://cruxcli.dev/favicon-96x96-v3.png",
      })
    ...
    getWslEnabled: async () => {
      ...
      return window.__CRUXCLI__!.wsl ?? false
    },
