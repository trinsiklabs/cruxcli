...
const CLI_INSTALL_DIR = ".cruxcli/bin"
const CLI_BINARY_NAME = "cruxcli"
...
export function getSidecarPath() {
  ...
  const path = app.isPackaged
    ? join(process.resourcesPath, `cruxcli-cli${suffix}`)
    : join(root, "../../resources", `cruxcli-cli${suffix}`)
...
export async function installCli(): Promise<string> {
  ...
  const tempScript = join(tmpdir(), "cruxcli-install.sh")
...
export function serve(hostname: string, port: number, password: string) {
  ...
  const env = {
    CRUXCLI_SERVER_USERNAME: "cruxcli",
    CRUXCLI_SERVER_PASSWORD: password,
  }
...
export function spawnCommand(args: string, extraEnv: Record<string, string>) {
  ...
  const envs = {
    ...base,
    CRUXCLI_EXPERIMENTAL_ICON_DISCOVERY: "true",
    CRUXCLI_EXPERIMENTAL_FILEWATCHER: "true",
    CRUXCLI_CLIENT: "desktop",
...
function buildCommand(args: string, env: Record<string, string>) {
  if (process.platform === "win32" && isWslEnabled()) {
    ...
    const script = [
      "set -e",
      'BIN="$HOME/.cruxcli/bin/cruxcli"',
      'if [ ! -x "$BIN" ]; then',
      `  curl -fsSL https://cruxcli.dev/install | bash -s -- --version ${shellEscape(version)} --no-modify-path`,
      "fi",
