...
const TAURI_APP_IDS: Record<string, string> = {
  dev: "ai.cruxcli.desktop.dev",
  beta: "ai.cruxcli.desktop.beta",
  prod: "ai.cruxcli.desktop",
}
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.cruxcli.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `cruxcli.settings.dat` is special: it maps to the `cruxcli.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"cruxcli.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // cruxcli.settings.dat → the electron settings store ("cruxcli.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "cruxcli.global.dat").
  const storeName = filename === "cruxcli.settings.dat" ? "cruxcli.settings" : filename
