...
Replace runtime Bun shell template-tag usage in `packages/cruxcli/src` with a unified `Process` API in `util/process.ts`.
...
- Runtime Bun `$` usage in `packages/cruxcli/src` is removed except:
