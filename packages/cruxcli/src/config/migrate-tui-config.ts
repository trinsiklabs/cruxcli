...
const TUI_SCHEMA_URL = "https://cruxcli.dev/tui.json"
...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from cruxcli.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const configs = await cruxcliFiles(input)
...
async function cruxcliFiles(input: { directories: string[]; managed: string }) {
  const project = Flag.CRUXCLI_DISABLE_PROJECT_CONFIG
    ? []
    : await ConfigPaths.projectFiles("cruxcli", Instance.directory, Instance.worktree)
  const files = [...project, ...ConfigPaths.fileInDirectory(Global.Path.config, "cruxcli")]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "cruxcli"))
  }
  if (Flag.CRUXCLI_CONFIG) files.push(Flag.CRUXCLI_CONFIG)
  files.push(...ConfigPaths.fileInDirectory(input.managed, "cruxcli"))
