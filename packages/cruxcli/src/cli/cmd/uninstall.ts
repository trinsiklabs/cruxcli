...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall cruxcli and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
  ...
  handler: async (args: UninstallArgs) => {
    ...
    UI.println(UI.logo("  "))
    UI.empty()
    prompts.intro("Uninstall CruxCLI")
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g cruxcli",
      pnpm: "pnpm uninstall -g cruxcli",
      bun: "bun remove -g cruxcli",
      yarn: "yarn global remove cruxcli",
      brew: "brew uninstall cruxcli",
      choco: "choco uninstall cruxcli",
      scoop: "scoop uninstall cruxcli",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result =
        method === "choco"
          ? await $`echo Y | choco uninstall cruxcli -y -r`.quiet().nothrow()
  ...
  UI.empty()
  prompts.log.success("Thank you for using CruxCLI!")
}
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# cruxcli") || content.includes(".cruxcli/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".cruxcli/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".cruxcli/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".cruxcli"))
    ) {
      continue
    }
