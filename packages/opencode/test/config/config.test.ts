...
const managedConfigDir = process.env.CRUXCLI_TEST_MANAGED_CONFIG_DIR!

afterEach(async () => {
  await fs.rm(managedConfigDir, { force: true, recursive: true }).catch(() => {})
})

async function writeManagedSettings(settings: object, filename = "cruxcli.json") {
  await fs.mkdir(managedConfigDir, { recursive: true })
  await Filesystem.write(path.join(managedConfigDir, filename), JSON.stringify(settings))
}

async function writeConfig(dir: string, config: object, name = "cruxcli.json") {
...
test("ignores legacy tui keys in cruxcli config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await writeConfig(dir, {
        $schema: "https://cruxcli.dev/config.json",
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        const content = await Filesystem.readText(path.join(tmp.path, "cruxcli.json"))
...
test("throws error for invalid JSON", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      await Filesystem.write(path.join(dir, "cruxcli.json"), "{ invalid json }")
    },
  })
  await Instance.provide({
    directory: tmp.path,
...
test("loads config from .cruxcli directory", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const cruxcliDir = path.join(dir, ".cruxcli")
      await fs.mkdir(cruxcliDir, { recursive: true })
      const agentDir = path.join(cruxcliDir, "agent")
...
test("loads agents from .cruxcli/agents (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const cruxcliDir = path.join(dir, ".cruxcli")
      await fs.mkdir(cruxcliDir, { recursive: true })

      const agentsDir = path.join(cruxcliDir, "agents")
...
test("loads commands from .cruxcli/command (singular)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const cruxcliDir = path.join(dir, ".cruxcli")
      await fs.mkdir(cruxcliDir, { recursive: true })

      const commandDir = path.join(cruxcliDir, "command")
...
test("loads commands from .cruxcli/commands (plural)", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      const cruxcliDir = path.join(dir, ".cruxcli")
      await fs.mkdir(cruxcliDir, { recursive: true })

      const commandsDir = path.join(cruxcliDir, "commands")
...
test("does not try to install dependencies in read-only CRUXCLI_CONFIG_DIR", async () => {
  ...
  const prev = process.env.CRUXCLI_CONFIG_DIR
  process.env.CRUXCLI_CONFIG_DIR = tmp.extra
  ...
  try {
    ...
    if (prev === undefined) delete process.env.CRUXCLI_CONFIG_DIR
    else process.env.CRUXCLI_CONFIG_DIR = prev
  }
})

test("installs dependencies in writable CRUXCLI_CONFIG_DIR", async () => {
  await using tmp = await tmpdir<string>({
    init: async (dir) => {
...
test("resolves scoped npm plugins in config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Filesystem.write(
        path.join(dir, "cruxcli.json"),
        JSON.stringify({ $schema: "https://cruxcli.dev/config.json", plugin: ["@scope/plugin"] }, null, 2),
      )
    },
  })
  ...
  await Instance.provide({
    ...
    fn: async () => {
      ...
      const baseUrl = pathToFileURL(path.join(tmp.path, "cruxcli.json")).href
...
test("merges plugin arrays from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      // Create a nested project structure with local .cruxcli config
      const projectDir = path.join(dir, "project")
      const cruxcliDir = path.join(projectDir, ".cruxcli")
      await fs.mkdir(cruxcliDir, { recursive: true })
      ...
      // Local .cruxcli config with different plugins
      await Filesystem.write(
        path.join(cruxcliDir, "cruxcli.json"),
        JSON.stringify({
          $schema: "https://cruxcli.dev/config.json",
...
test("deduplicates duplicate plugins from global and local configs", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .cruxcli config with some overlapping plugins
      await Filesystem.write(
        path.join(cruxcliDir, "cruxcli.json"),
        JSON.stringify({
          $schema: "https://cruxcli.dev/config.json",
...
// Note: preload.ts sets CRUXCLI_TEST_MANAGED_CONFIG which Global.Path.managedConfig uses
...
test("local .cruxcli config can override MCP from project config", async () => {
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      // Local .cruxcli directory config enables it
      const cruxcliDir = path.join(dir, ".cruxcli")
      await fs.mkdir(cruxcliDir, { recursive: true })
      await Filesystem.write(
        path.join(cruxcliDir, "cruxcli.json"),
...
test("project config overrides remote well-known config", async () => {
  ...
  const mockFetch = mock((url: string | URL | Request) => {
    ...
    if (urlStr.includes(".well-known/cruxcli")) {
      fetchedUrl = urlStr
  ...
  try {
    ...
    await Instance.provide({
      ...
      fn: async () => {
        ...
        expect(fetchedUrl).toBe("https://example.com/.well-known/cruxcli")
...
describe("getPluginName", () => {
  ...
  test("extracts name from npm package with version", () => {
    expect(Config.getPluginName("oh-my-cruxcli@2.4.3")).toBe("oh-my-cruxcli")
  ...
  test("extracts name from scoped npm package", () => {
    ...
    expect(Config.getPluginName("@cruxcli/plugin@2.0.0")).toBe("@cruxcli/plugin")
  })
...
describe("deduplicatePlugins", () => {
  ...
  test("prefers local file over npm package with same name", () => {
    const plugins = ["oh-my-cruxcli@2.4.3", "file:///project/.cruxcli/plugin/oh-my-cruxcli.js"]

    const result = Config.deduplicatePlugins(plugins)

    expect(result.length).toBe(1)
    expect(result[0]).toBe("file:///project/.cruxcli/plugin/oh-my-cruxcli.js")
  })
  ...
  test("local plugin directory overrides global cruxcli.json plugin", async () => {
    await using tmp = await tmpdir({
      init: async (dir) => {
        const projectDir = path.join(dir, "project")
        const cruxcliDir = path.join(projectDir, ".cruxcli")
        const pluginDir = path.join(cruxcliDir, "plugin")
        await fs.mkdir(pluginDir, { recursive: true })
...
describe("CRUXCLI_DISABLE_PROJECT_CONFIG", () => {
  test("skips project config files when flag is set", async () => {
    const originalEnv = process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"]
    process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      ...
      if (originalEnv === undefined) {
        delete process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"]
      } else {
        process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"] = originalEnv
      }
    }
  })

  test("skips project .cruxcli/ directories when flag is set", async () => {
    const originalEnv = process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"]
    process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"] = "true"
    ...
    try {
      await using tmp = await tmpdir({
        init: async (dir) => {
          // Create a .cruxcli directory with a command
          const cruxcliDir = path.join(dir, ".cruxcli", "command")
          await fs.mkdir(cruxcliDir, { recursive: true })
          await Filesystem.write(path.join(cruxcliDir, "test-cmd.md"), "# Test Command\nThis is a test command.")
        },
      })
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          ...
          // Project .cruxcli should NOT be in directories list
          const hasProjectCruxcli = directories.some((d) => d.startsWith(tmp.path))
          expect(hasProjectCruxcli).toBe(false)
        },
      })
    } finally {
      if (originalEnv === undefined) {
  ...
  test("skips relative instructions with warning when flag is set but no config dir", async () => {
    const originalDisable = process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CRUXCLI_CONFIG_DIR"]

    try {
      // Ensure no config dir is set
      delete process.env["CRUXCLI_CONFIG_DIR"]
      process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"] = "true"
      ...
      if (originalDisable === undefined) {
        ...
        process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"] = originalDisable
      }
      if (originalConfigDir === undefined) {
        delete process.env["CRUXCLI_CONFIG_DIR"]
      } else {
        process.env["CRUXCLI_CONFIG_DIR"] = originalConfigDir
      }
    }
  })

  test("CRUXCLI_CONFIG_DIR still works when flag is set", async () => {
    const originalDisable = process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"]
    const originalConfigDir = process.env["CRUXCLI_CONFIG_DIR"]
    ...
    try {
      ...
      process.env["CRUXCLI_DISABLE_PROJECT_CONFIG"] = "true"
      process.env["CRUXCLI_CONFIG_DIR"] = configDirTmp.path

      await Instance.provide({
        directory: projectTmp.path,
        fn: async () => {
          const config = await Config.get()
          // Should load from CRUXCLI_CONFIG_DIR, not project
...
describe("CRUXCLI_CONFIG_CONTENT token substitution", () => {
  test("substitutes {env:} tokens in CRUXCLI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CRUXCLI_CONFIG_CONTENT"]
    const originalTestVar = process.env["TEST_CONFIG_VAR"]
    process.env["TEST_CONFIG_VAR"] = "test_api_key_12345"
    process.env["CRUXCLI_CONFIG_CONTENT"] = JSON.stringify({
      $schema: "https://cruxcli.dev/config.json",
    ...
    try {
      ...
      if (originalEnv !== undefined) {
        process.env["CRUXCLI_CONFIG_CONTENT"] = originalEnv
      } else {
        delete process.env["CRUXCLI_CONFIG_CONTENT"]
      }
  ...
  test("substitutes {file:} tokens in CRUXCLI_CONFIG_CONTENT", async () => {
    const originalEnv = process.env["CRUXCLI_CONFIG_CONTENT"]
