...
export const PrCommand = cmd({
  command: "pr <number>",
  describe: "fetch and checkout a GitHub PR branch, then run cruxcli",
  builder: (yargs) =>
      ...
      async fn() {
        ...
        if (prInfoResult.exitCode === 0) {
          ...
          if (prInfoText.trim()) {
            ...
            // Check for cruxcli session link in PR body
            if (prInfo && prInfo.body) {
              const sessionMatch = prInfo.body.match(/https:\/\/opncd\.ai\/s\/([a-zA-Z0-9_-]+)/)
              if (sessionMatch) {
                const sessionUrl = sessionMatch[0]
                UI.println(`Found cruxcli session: ${sessionUrl}`)
                UI.println(`Importing session...`)

                const importResult = await $`cruxcli import ${sessionUrl}`.nothrow()
        ...
        UI.println()
        UI.println("Starting cruxcli...")
        UI.println()

        // Launch cruxcli TUI with session ID if available
        const { spawn } = await import("child_process")
        const cruxcliArgs = sessionId ? ["-s", sessionId] : []
        const cruxcliProcess = spawn("cruxcli", cruxcliArgs, {
          stdio: "inherit",
          cwd: process.cwd(),
        })

        await new Promise<void>((resolve, reject) => {
          cruxcliProcess.on("exit", (code) => {
            if (code === 0) resolve()
            else reject(new Error(`cruxcli exited with code ${code}`))
          })
          cruxcliProcess.on("error", reject)
        })
      },
    })
  },
