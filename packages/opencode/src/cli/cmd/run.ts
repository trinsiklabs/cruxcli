...
import { createCruxcliClient, type Message, type CruxcliClient, type ToolPart } from "@cruxcli/sdk/v2"
...
export const RunCommand = cmd({
  command: "run [message..]",
  describe: "run cruxcli with a message",
  builder: (yargs: Argv) => {
    return yargs
      .positional("message", {
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running cruxcli server (e.g., http://localhost:4096)",
      })
      .option("dir", {
        type: "string",
  ...
  handler: async (args) => {
    ...
    async function session(sdk: CruxcliClient) {
    ...
    async function share(sdk: CruxcliClient, sessionID: string) {
      const cfg = await sdk.config.get()
      if (!cfg.data) return
      if (cfg.data.share !== "auto" && !Flag.CRUXCLI_AUTO_SHARE && !args.share) return
    ...
    async function execute(sdk: CruxcliClient) {
    ...
    if (args.attach) {
      const sdk = createCruxcliClient({ baseUrl: args.attach, directory })
    ...
    await bootstrap(process.cwd(), async () => {
      ...
      const sdk = createCruxcliClient({ baseUrl: "http://cruxcli.internal", fetch: fetchFn })
      await execute(sdk)
    })
  },
})
