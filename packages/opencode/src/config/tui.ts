...
export namespace TuiConfig {
  ...
  function customPath() {
    return Flag.CRUXCLI_TUI_CONFIG
  }

  const state = Instance.state(async () => {
    let projectFiles = Flag.CRUXCLI_DISABLE_PROJECT_CONFIG
      ? []
    ...
    projectFiles = Flag.CRUXCLI_DISABLE_PROJECT_CONFIG
      ? []
      : await ConfigPaths.projectFiles("tui", Instance.directory, Instance.worktree)
    ...
    for (const dir of unique(directories)) {
      if (!dir.endsWith(".cruxcli") && dir !== Flag.CRUXCLI_CONFIG_DIR) continue
  ...
  async function load(text: string, configFilepath: string): Promise<Info> {
    ...
    // (mirroring the old cruxcli.json shape) still get their settings applied.
    const normalized = (() => {
