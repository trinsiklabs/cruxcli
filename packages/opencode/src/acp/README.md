...
This directory contains a clean, protocol-compliant implementation of the [Agent Client Protocol](https://agentclientprotocol.com/) for cruxcli.
...
- **`session.ts`** - Session state management
  ...
  - Maps ACP sessions to internal cruxcli sessions
...
cruxcli acp --cwd /path/to/project
```
...
```bash
CRUXCLI_ENABLE_QUESTION_TOOL=1 cruxcli acp
```
...
- **Terminal Integration**: Full terminal support via cruxcli's bash tool
...
echo '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}' | cruxcli acp
```
...
ACP sessions map cleanly to cruxcli's internal session model:
