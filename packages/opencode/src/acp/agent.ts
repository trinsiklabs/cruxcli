...
import type { AssistantMessage, Event, CruxcliClient, SessionMessageResponse, ToolPart } from "@cruxcli/sdk/v2"
...
export namespace ACP {
  ...
  export async function init({ sdk: _sdk }: { sdk: CruxcliClient }) {
    return {
  ...
  export class Agent implements ACPAgent {
    ...
    async initialize(params: InitializeRequest): Promise<InitializeResponse> {
      ...
      const authMethod: AuthMethod = {
        description: "Run `cruxcli auth login` in the terminal",
        name: "Login with cruxcli",
        id: "cruxcli-login",
      }
    ...
    private async processMessage(message: SessionMessageResponse) {
      ...
      for (const part of message.parts) {
        if (part.type === "tool") {
          ...
          // CruxCLI stores files internally as { type: "file", url, filename, mime }.
  ...
  async function defaultModel(config: ACPConfig, cwd?: string) {
    ...
    const cruxcliProvider = providers.find((p) => p.id === "cruxcli")
    if (cruxcliProvider) {
      if (cruxcliProvider.models["big-pickle"]) {
        return { providerID: "cruxcli", modelID: "big-pickle" }
      }
      const [best] = Provider.sort(Object.values(cruxcliProvider.models))
      if (best) {
        return {
