...
import { createCruxcli, type ToolPart } from "@cruxcli/sdk"
...
console.log("🚀 Starting cruxcli server...")
const cruxcli = await createCruxcli({
  port: 0,
})
console.log("✅ Cruxcli server ready")

const sessions = new Map<string, { client: any; server: any; sessionId: string; channel: string; thread: string }>()
;(async () => {
  const events = await cruxcli.client.event.subscribe()
...
app.message(async ({ message, say }) => {
  ...
  if (!session) {
    console.log("🆕 Creating new cruxcli session...")
    const { client, server } = cruxcli
    ...
    console.log("✅ Created cruxcli session:", createResult.data.id)
  ...
  console.log("📝 Sending to cruxcli:", message.text)
  const result = await session.client.session.prompt({
    path: { id: session.sessionId },
    body: { parts: [{ type: "text", text: message.text }] },
  })

  console.log("📤 Cruxcli response:", JSON.stringify(result, null, 2))
