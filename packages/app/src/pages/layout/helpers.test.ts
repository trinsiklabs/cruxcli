...
import { type Session } from "@cruxcli/sdk/v2/client"
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("cruxcli://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("cruxcli://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("cruxcli://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("cruxcli://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("cruxcli://open-project")).toBeUndefined()
    expect(parseDeepLink("cruxcli://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "cruxcli://open-project?directory=/a",
      "cruxcli://other?directory=/b",
      "cruxcli://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("cruxcli://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("cruxcli://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("cruxcli://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("cruxcli://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "cruxcli://new-session?directory=/a",
      "cruxcli://open-project?directory=/b",
      "cruxcli://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CRUXCLI__: {
        deepLinks: ["cruxcli://open-project?directory=/a"],
      },
    } as unknown as Window & { __CRUXCLI__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["cruxcli://open-project?directory=/a"])
